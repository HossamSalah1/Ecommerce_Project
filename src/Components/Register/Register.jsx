...
function Register() {
    ...
    let [isloading, setIsLoding] = useState(0)

    async function handelSubmit(value) {
        setIsLoding(1);
        let response = await axios.post(`https://ecommerce.routemisr.com/api/v1/auth/signup`, value).catch((err) => {
            setIsLoding(0)
    ...
    let validationSchema = yup.object({
        ...
        phone: yup
            .string()
            .matches(phoneRejex, "phone is invalid")
    ...
    return <>
        <div className="w-75 py-5 mx-auto ">
            ...
            <form onSubmit={formik.handleSubmit}>
                ...
                <input className='form-control ' type='text' name='name' value={formik.values.name} onChange={formik.handleChange} id='name' onBlur={formik.handleBlur} />
                {formik.errors.name && formik.touched.name ? <div className="alert alert-info p-2 mt-2 ">{formik.errors.name}</div> : null}
                ...
                <input className='form-control ' name='phone' value={formik.values.phone} onChange={formik.handleChange} id='phone' onBlur={formik.handleBlur} />
                {formik.errors.phone && formik.touched.phone ? <div className="alert alert-info p-2 mt-2 ">{formik.errors.phone}</div> : null}
                ...
                <input type='password' autoComplete='true' className='form-control ' name='rePassword' value={formik.values.rePassword} onChange={formik.handleChange} id='rePassword' onBlur={formik.handleBlur} />
                {formik.errors.rePassword && formik.touched.rePassword ? <div className="alert alert-info p-2 mt-2 ">{formik.errors.rePassword}</div> : null}
